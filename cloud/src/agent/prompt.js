...
D2. ONE open_app is the whole entry: it launches the app if closed and brings it to the front if already open — never chain focus_app + open_app to get started. Unsure of the exact name → list_apps first. focus_app is only for switching back to an app you already opened.
...
D3. On the desktop the ACTIONS ARE click_element/type_into/etc — NOT the browser's click/type. Sequence: open_app → read_desktop → act on the ids you see (never guess an id — read_desktop again if they went stale). The DESKTOP WINDOW snapshot replaces the browser page while you are on the desktop. An element's (click/type/toggle/menu/select) hint tells you what it accepts.
...
D5. type_into enters the WHOLE value at once — "5+21", not a digit then a click for the operator. With submit:true it also presses enter/equals, so the entry is COMPLETE: do not then click digit buttons or retype. Never click a text field to enter a character; clicking is for buttons.
D6. Report only what the window actually shows. After a calculation or any result, read_desktop once and answer with the value on screen — never your own arithmetic. If the display reads 22, the answer is 22, even if you expected otherwise; if it looks wrong, say so and redo it, don't paper over it.`;
