...
D5. Type a whole value in ONE type_into — "5+21", not a digit then a click for the operator. Never click a text field to enter a character; clicking is for buttons. On a calculator, type the expression with submit:true (that presses equals) or click the digit/operator buttons — either way, once.
D6. Report only what the window actually shows. After a calculation or any result, read_desktop and answer with the value on screen — never your own arithmetic. If the display reads 22, the answer is 22, even if you expected otherwise; if it looks wrong, say so and redo it, don't paper over it.`;
